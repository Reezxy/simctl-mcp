...
export async function waitForScreenStable(
  ...
  while (true) {
    ...
    let diffResult: Awaited<ReturnType<typeof diffScreenshots>>;
    try {
...
export const waitTools: McpToolDef[] = [
  {
    name: "wait_for_element",
    description:
      "Polls the accessibility tree until an element with the given label appears or the timeout is exceeded. " +
    ...
    handler: async (args) => {
      ...
      const timeout = (args["timeout_ms"] as number | undefined) ?? DEFAULT_ELEMENT_TIMEOUT_MS;
      const poll = (args["poll_interval_ms"] as number | undefined) ?? DEFAULT_POLL_INTERVAL_MS;
      try {
...

  {
    ...
    handler: async (args) => {
      const timeout = (args["timeout_ms"] as number | undefined) ?? DEFAULT_STABLE_TIMEOUT_MS;
      try {
...

  {
    ...
    description:
      "Waits until an element disappears from the accessibility tree. " +
