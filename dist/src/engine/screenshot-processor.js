...
function timestampedFilename(screenName) {
    const now = new Date();
    const ts = now
        .toISOString()
        .replace(/[-:]/g, "")
 ...
 */
export async function processScreenshot(sourcePngPath, screenName) {
 ...
 */
export async function diffScreenshots(baselinePath, comparePath, thresholdPercent = 2) {
    await ensureScreenshotsDir();
 ...
 */
export async function getScreenshotStats(filePath) {
