...
export function buildBugSection(bugs) {
    if (bugs.length === 0) {
...
export function buildCoverageTable(nodes) {
    if (nodes.length === 0) {
    ...
    const lines = [
        ...
        "|--------|--------|--------|-------|",
        ...sorted.map((n) => `| ${n.name} | ${resultLabel(n.result)} | ${n.visitCount} | ${n.notes ?? ""} |`),
    ];
    return lines.join("\n");
}
export function buildRecommendations(stats, bugs) {
    const items = [];
    // Crashes first
    if (stats.crashes > 0) {
        const crashBugs = bugs.filter((b) => b.title.toLowerCase().includes("crash"));
        for (const b of crashBugs) {
            items.push(`**[Must fix before release]** Crash on "${b.screen}": ${b.title}`);
        }
    }
    // Critical bugs
    ...
    if (highs.length > 0) {
        items.push(`**[High priority]** Address ${highs.length} high-severity bug(s): ` +
            highs.map((b) => b.title).join("; "));
    }
    // Blocked screens
    if (stats.screensBlocked > 0) {
        items.push(`**[Investigate]** ${stats.screensBlocked} screen(s) were blocked — ` +
            "check accessibility labels and navigation paths");
    }
    // Coverage
    if (stats.coveragePercent < 80 && stats.screensDiscovered > 0) {
        items.push(`**[Coverage]** Only ${stats.coveragePercent}% of screens were tested — ` +
...
export function buildMarkdownReport(appName = "Unknown App", appVersion = "") {
...
export function buildJsonReport(appName = "Unknown App", appVersion = "") {
    const stats = getStats();
    const { nodes, edges } = getGraph();
    const bugs = getBugs();
    return {
...
// ── Public API ────────────────────────────────────────────────────────────────
export async function generateReport(format, appName, appVersion) {
    if (format === "json") {
