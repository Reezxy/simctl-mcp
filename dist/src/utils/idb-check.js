...
export async function getBackend() {
    ...
    if (!xcrunAvailable) {
        throw new Error("Neither idb nor xcrun simctl is available. " +
            "Install Xcode Command Line Tools (`xcode-select --install`) " +
            "or idb (`pip3 install fb-idb`) to continue.");
    }
    // Warn once to stderr so the MCP server can surface it
    if (process.env.NODE_ENV !== "test") {
        console.warn("[ios-simulator-mcp] idb not found — falling back to xcrun simctl. " +
...
export async function getIdbPath() {
    const status = await getBackend();
 ...
 */
export async function idbCmd(args) {
...
//# sourceMappingURL=idb-check.js.map