...
function parseCrashFile(content, filePath) {
...
// ── Core logic ────────────────────────────────────────────────────────────────
export async function getConsoleLogs(lines = 100, filter) {
    const result = await exec("xcrun simctl spawn booted log show --last 30s --style compact", { timeoutMs: 15_000 });
...
export async function getCrashLogs(bundleId) {
    let files;
    try {
...
export async function getNetworkCalls(sinceMs) {
...
export async function startMockServer(port = 3210, routesConfig) {
    if (mockServerState.running) {
        return {
    ...
    const starter = globalThis["__mockServerStart__"];
    if (!starter) {
        throw new Error("Mock server module not loaded. " +
...
export const diagnosticTools = [
    {
        ...
        description: "Returns the last N lines of the simulator's console output from the past 30 seconds. " +
    ...
    {
        name: "get_crash_logs",
        description: "Returns crash reports from ~/Library/Logs/DiagnosticReports. " +
    ...
    },
    {
        ...
        description: "Returns all HTTP requests the app made to the mock server. " +
        ...
        inputSchema: {
            ...
            properties: {
                since_ms: {
                    type: "number",
                    description: "Only return calls after this Unix timestamp (ms). Omit for all calls.",
                },
            },
        },
        handler: async (args) => {
            const sinceMs = args["since_ms"];
            try {
                const result = await getNetworkCalls(sinceMs);
                if (!result.serverRunning) {
                    return toolError("Mock server is not running. Call start_mock_server first.");
                }
    ...
    },
    {
        ...
        description: "Starts the bundled HTTP mock server on the given port (default 3210). " +
        ...
        handler: async (args) => {
            ...
            const routes = args["routes_config"];
            try {
    ...
    },
    {
        ...
        description: "Stops the mock server and returns a summary of all HTTP calls that were recorded. " +
