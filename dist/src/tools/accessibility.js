...
import { findElement, filterInteractive, extractVisibleText, } from "../engine/element-finder.js";
...
function detectScreenType(elements) {
    ...
    // Auth
    if (AUTH_KEYWORDS.some((k) => allText.includes(k)) &&
    ...
    // Form: 2+ text fields
    const fieldCount = types.filter((t) => ["textfield", "securetextfield", "searchfield"].includes(t)).length;
    if (fieldCount >= 2)
        return "form";
    // List: many cells / rows
    const cellCount = types.filter((t) => ["cell", "row", "staticcell"].includes(t)).length;
    if (cellCount >= 3)
        return "list";
    // Onboarding
...
function suggestActions(screenType, interactive) {
...
export async function getAccessibilityTree() {
    ...
    if (backend.backend === "idb") {
        ...
        if (isExecError(result)) {
            throw new Error(`idb accessibility tree failed: ${result.message}. ` +
...
export const accessibilityTools = [
    {
        ...
        description: "Returns the full UI accessibility tree of the current screen as a flat array of elements. " +
    ...
    },
    {
        ...
        description: "High-level description of the current screen. Returns: screen title, detected screen type " +
    ...
    {
        name: "find_element",
        description: "Searches the accessibility tree for an element matching the query. " +
        ...
        handler: async (args) => {
            ...
            const strategy = args["strategy"] ?? "fuzzy";
            try {
                const { elements } = await getAccessibilityTree();
