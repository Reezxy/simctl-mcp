...
const HID_DELETE = 42;
// ── Helpers ───────────────────────────────────────────────────────────────────
async function requireIdb() {
    const backend = await getBackend();
    if (backend.backend !== "idb") {
        throw new Error("Interaction tools require idb. " +
...
// ── Core logic ────────────────────────────────────────────────────────────────
export async function tapByLabel(label, fallbackScreenshot = false) {
    ...
    if (!found.found || !found.element) {
        ...
        return toolError(`Element not found: "${label}". ${found.suggestion ?? "Try take_screenshot to see current state."}`);
    }
    ...
    const result = await exec(`${idbPath} ui tap ${cx} ${cy}`, { timeoutMs: 10_000 });
...
export async function inputText(text, clearFirst = false) {
    const idbPath = await requireIdb();
    if (clearFirst) {
...
export async function swipe(direction, distancePercent = 50, screenWidth = DEFAULT_SCREEN_WIDTH, screenHeight = DEFAULT_SCREEN_HEIGHT) {
    const idbPath = await requireIdb();
    ...
    const result = await exec(`${idbPath} ui swipe ${x1} ${y1} ${x2} ${y2}`, { timeoutMs: 10_000 });
    if (isExecError(result)) {
        throw new Error(`Swipe ${direction} failed: ${result.message}`);
    }
}
export async function scrollToElement(label, maxScrolls = 10) {
...
export async function pressHardwareButton(button) {
    ...
    if (button === "rotate_left" || button === "rotate_right") {
        ...
        const result = await exec(`xcrun simctl ui booted rotate ${direction}`, { timeoutMs: 10_000 });
        if (isExecError(result)) {
    ...
    const result = await exec(`xcrun simctl ui booted button ${simctlBtn}`, { timeoutMs: 10_000 });
    if (isExecError(result)) {
        throw new Error(`Hardware button "${button}" failed: ${result.message}`);
    }
}
export async function longPress(label, durationMs = 1000) {
    const idbPath = await requireIdb();
    const { elements } = await getAccessibilityTree();
    const found = findElement(elements, label, "fuzzy");
    if (!found.found || !found.element) {
        throw new Error(`Element not found for long press: "${label}". ${found.suggestion ?? ""}`);
    }
    const { x, y } = centerOf(found.element);
    const durationSec = (durationMs / 1000).toFixed(2);
    const result = await exec(`${idbPath} ui long-press ${x} ${y} ${durationSec}`, { timeoutMs: durationMs + 5_000 });
    if (isExecError(result)) {
        throw new Error(`Long press on "${label}" failed: ${result.message}`);
    }
}
export async function drag(fromLabel, toLabel) {
    const idbPath = await requireIdb();
    const { elements } = await getAccessibilityTree();
    ...
    const result = await exec(`${idbPath} ui swipe ${from.x} ${from.y} ${to.x} ${to.y}`, { timeoutMs: 10_000 });
    if (isExecError(result)) {
...
export const interactionTools = [
    {
        name: "tap_by_label",
        description: "Primary tap method. Finds a UI element by label (fuzzy-matched), calculates its center, and taps it. " +
        ...
        handler: async (args) => {
            ...
            const fallback = args["fallback_screenshot"] ?? false;
            try {
    ...
    {
        name: "tap_at",
        description: "Taps at explicit screen coordinates. Use only when tap_by_label fails — prefer label-based tapping. " +
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            const distance = args["distance_percent"] ?? 50;
            try {
    ...
    },
    {
        ...
        description: "Scrolls down repeatedly until the named element appears in the accessibility tree. " +
        ...
        handler: async (args) => {
            const label = args["label"];
            const maxScrolls = args["max_scrolls"] ?? 10;
            try {
    ...
    },
    {
        ...
        description: "Simulates a hardware button press on the simulator. " +
    ...
    {
        name: "long_press",
        description: "Performs a long press on the element with the given label. " +
        ...
        handler: async (args) => {
            const label = args["label"];
            const duration = args["duration_ms"] ?? 1000;
            try {
    ...
    {
        name: "drag",
        description: "Drags from one element to another by label. Useful for reordering lists, " +
        ...
        handler: async (args) => {
            const fromLabel = args["from_label"];
            const toLabel = args["to_label"];
            try {
