...
const DIAG_DIR = path.join(os.homedir(), "Library", "Logs", "DiagnosticReports");
function parseCrashSummary(content, filePath) {
    // .ips files are JSON
    if (filePath.endsWith(".ips")) {
        try {
            const json = JSON.parse(content);
            const exc = json["exception"];
            const exType = exc?.["type"] ?? "unknown";
            const exSignal = exc?.["signal"] ?? exc?.["subtype"] ?? "";
            const threads = json["threads"] ?? [];
            const crashed = threads.find((t) => t["triggered"] === true || t["name"] === "Crashed");
            const frames = crashed?.["frames"] ?? [];
            const top5 = frames
                .slice(0, 5)
    ...
    const crashedIdx = lines.findIndex((l) => l.includes("Crashed:") || l.includes("Thread 0 Crashed"));
    const frames = crashedIdx >= 0
        ? lines
...
export async function assertElementExists(label, shouldExist = true) {
    try {
        ...
        return {
            passed: false,
            message: `Could not check element existence: ${err.message}`,
        };
    }
}
export async function assertTextEquals(label, expectedText) {
    try {
        const { elements } = await getAccessibilityTree();
        ...
        return {
            passed: false,
            message: `Could not assert text: ${err.message}`,
        };
    }
}
export async function assertElementEnabled(label, shouldBeEnabled = true) {
    try {
        const { elements } = await getAccessibilityTree();
        const result = findElement(elements, label, "fuzzy");
        if (!result.found || !result.element) {
        ...
        return {
            passed: false,
            message: `Could not assert enabled state: ${err.message}`,
        };
    }
}
export async function assertNoCrash() {
    ...
    const crashFiles = files.filter((f) => f.endsWith(".crash") || f.endsWith(".ips"));
    const recentCrashes = [];
...
export async function assertNoErrorInLogs(patterns = DEFAULT_ERROR_PATTERNS) {
    // Pull last 10 seconds of simulator console output
    const result = await exec("xcrun simctl spawn booted log show --last 10s --style compact", { timeoutMs: 15_000 });
    ...
    const matched = lines.filter((line) => patterns.some((p) => line.toLowerCase().includes(p.toLowerCase())));
    return {
...
export function resetCrashBaseline() {
    lastCrashCheckMs = Date.now();
}
...
export const assertionTools = [
    {
        ...
        description: "Checks whether an element with the given label is present in the accessibility tree. " +
        ...
        handler: async (args) => {
            ...
            const shouldExist = args["should_exist"] ?? true;
    ...
    },
    {
        ...
        description: "Finds an element by label and checks that its text content (label or value) matches expected_text. " +
    ...
    },
    {
        ...
        description: "Checks whether an element is enabled (interactive). " +
        ...
        handler: async (args) => {
            const label = args["label"];
            const shouldBeEnabled = args["should_be_enabled"] ?? true;
    ...
    },
    {
        ...
        description: "Checks crash logs for any app crash since the last time this tool was called. " +
    ...
    },
    {
        ...
        description: "Scans the last 10 seconds of simulator console output for error patterns. " +
        ...
        inputSchema: {
            ...
            properties: {
                patterns: {
                    type: "array",
                    items: { type: "string" },
                    description: "Strings to search for in logs (case-insensitive). " +
    ...
    },
    {
        ...
        description: "Diffs the current screen against a saved baseline screenshot. " +
