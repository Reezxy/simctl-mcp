...
// ── Core logic ────────────────────────────────────────────────────────────────
export async function injectUserDefaults(bundleId, key, value) {
    const { flag, str } = plistTypeFlag(value);
    await execOrThrow(`xcrun simctl spawn booted defaults write "${bundleId}" "${key}" ${flag} "${str}"`, { timeoutMs: 10_000 });
}
export async function readUserDefaults(bundleId, key) {
    const result = await exec(`xcrun simctl spawn booted defaults read "${bundleId}" "${key}"`, { timeoutMs: 10_000 });
    if (isExecError(result))
        return null;
    return result.stdout.trim() || null;
}
export async function setKeychainValue(account, service, value) {
    const backend = await getBackend();
    if (backend.backend !== "idb") {
        throw new Error("Keychain injection requires idb. Install idb (pip3 install fb-idb).");
    }
    await execOrThrow(`${backend.idbPath} keychain add --account "${account}" --service "${service}" --value "${value}"`, { timeoutMs: 10_000 });
}
export async function resetAppState(bundleId) {
    const steps = [];
    // 1. Terminate
    const termResult = await exec(`xcrun simctl terminate booted "${bundleId}"`, { timeoutMs: 10_000 });
    steps.push(isExecError(termResult)
    ...
    if (!isExecError(containerResult)) {
        ...
        for (const dir of ["Library", "Documents", "tmp"]) {
            ...
            steps.push(isExecError(rm) ? `clear ${dir}: failed` : `clear ${dir}: ok`);
        }
    }
    else {
    ...
    const privResult = await exec(`xcrun simctl privacy booted reset all "${bundleId}"`, { timeoutMs: 10_000 });
    steps.push(isExecError(privResult)
...
export async function setLocation(latitude, longitude) {
    await execOrThrow(`xcrun simctl location booted set ${latitude} ${longitude}`, { timeoutMs: 10_000 });
}
export async function setPermissions(bundleId, permission, value) {
    await execOrThrow(`xcrun simctl privacy booted ${value} ${permission} "${bundleId}"`, { timeoutMs: 10_000 });
}
// ── MCP tool definitions ──────────────────────────────────────────────────────
export const authTools = [
    {
        name: "inject_user_defaults",
        description: "Writes a value to the app's NSUserDefaults (plist preferences). " +
    ...
    },
    {
        ...
        description: "Reads a value from the app's NSUserDefaults. " +
    ...
    },
    {
        ...
        description: "Injects a keychain entry so the app can start in a logged-in state. " +
    ...
    },
    {
        ...
        description: "Full app state reset: terminates the app, clears the data container " +
    ...
    {
        name: "set_location",
        description: "Sets the simulated GPS location for the booted simulator. " +
    ...
    },
    {
        ...
        description: "Grants, revokes, or resets a specific app permission. " +
